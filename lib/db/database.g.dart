...
class $SpotsTable extends Spots with TableInfo<$SpotsTable, Spot> {
  ...
  late final GeneratedColumn<String> category = GeneratedColumn<String>(
      ...
      defaultConstraints:
          GeneratedColumn.constraintIsAlways('REFERENCES categories (name)'));
...
final class $$CategoriesTableReferences
  ...
  static MultiTypedResultKey<$SpotsTable, List<Spot>> _spotsRefsTable(
          _$AppDatabase db) =>
      MultiTypedResultKey.fromTable(db.spots,
          aliasName:
              $_aliasNameGenerator(db.categories.name, db.spots.category));

  $$SpotsTableProcessedTableManager get spotsRefs {
    final manager = $$SpotsTableTableManager($_db, $_db.spots).filter(
        (f) => f.category.name.sqlEquals($_itemColumn<String>('name')!));
...
class $$CategoriesTableTableManager extends RootTableManager<
    ...
    Category,
    PrefetchHooks Function({bool spotsRefs})> {
  $$CategoriesTableTableManager(_$AppDatabase db, $CategoriesTable table)
      : super(TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              ...
              .toList(),
          prefetchHooksCallback: ({spotsRefs = false}) {
            return PrefetchHooks(
              db: db,
              explicitlyWatchedTables: [if (spotsRefs) db.spots],
              addJoins: null,
              getPrefetchedDataCallback: (items) async {
                return [
                  if (spotsRefs)
                    await $_getPrefetchedData<Category, $CategoriesTable, Spot>(
                        currentTable: table,
                        referencedTable:
                            $$CategoriesTableReferences._spotsRefsTable(db),
                        managerFromTypedResult: (p0) =>
                            $$CategoriesTableReferences(db, table, p0)
                                .spotsRefs,
                        referencedItemsForCurrentItem:
                            (item, referencedItems) => referencedItems
                                .where((e) => e.category == item.name),
                        typedResults: items)
                ];
              },
            );
...
final class $$SpotsTableReferences
  ...
  static $CategoriesTable _categoryTable(_$AppDatabase db) => db.categories
      .createAlias($_aliasNameGenerator(db.spots.category, db.categories.name));
              ...
              addJoins: <
                ...
                if (category) {
                  state = state.withJoin(
                    ...
                    referencedTable: $$SpotsTableReferences._categoryTable(db),
                    referencedColumn:
                        $$SpotsTableReferences._categoryTable(db).name,
                  ) as T;
                }
